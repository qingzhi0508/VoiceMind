...
- Rust 端新增 `get_version` 命令，接收 `app: tauri::AppHandle` 参数，通过 `app.config().version` 返回 `tauri.conf.json` 中定义的版本号
- 版本来源以 `tauri.conf.json` 为准（与打包配置一致）
...
- 从 `assets` 数组中找到 `.msi` 文件：优先匹配包含 `x64` 的文件，若无则取第一个 `.msi` 文件的 `browser_download_url`
...
**缓存策略：**
- 将上次检查结果（版本号、下载链接、检查时间）存入 `localStorage`
- 启动检查：如果距离上次成功检查不足 4 小时，跳过 API 调用，直接使用缓存结果
- 手动检查：始终发起 API 请求
...
- 下载按钮调用 `window.__TAURI__.shell.open(url)` 在系统浏览器中打开 MSI 下载链接

**启动通知条：**
- 在 `#banner` 元素之后新增独立的 `#update-banner` 元素
- 有新版本时显示："发现新版本 v{x.y.z}，点击查看"
- 点击通知条导航到 About 页
- 关闭后本次启动不再显示（内存状态，不持久化）
...
```js
// state.js 新增字段
currentVersion: '',        // 从 Rust 获取的当前版本号
updateInfo: null,          // { version, downloadUrl, body } 或 null
updateChecking: false,     // 是否正在检查
updateBannerDismissed: false  // 本次启动通知条是否已关闭
```

### 5. Tauri v2 权限配置

需要创建 `src-tauri/capabilities/default.json`，授予 shell 插件打开链接的权限：
...
| `src-tauri/src/main.rs` | 在 `invoke_handler` 中注册 `get_version` |
| `src-tauri/capabilities/default.json` | 新建，配置 `shell:allow-open` 权限 |
| `src/app/state.js` | 新增 `currentVersion`、`updateInfo`、`updateChecking`、`updateBannerDismissed` 字段 |
| `src/app/app.js` | 新增更新检查逻辑、UI 渲染、事件绑定 |
| `src/index.html` | About 页版本号动态化、新增检查更新按钮、新增更新通知条 |
